...
static bool run_headless(Dataloader::path_vector_t const& roots, bool run_tests) {
	...
	if (game_manager.get_instance_manager()) {
		...
		std::vector<ProvinceInstance> const& provinces = game_manager.get_instance_manager()->get_map_instance().get_province_instances();
		std::vector<ProvinceInstance> const& provinces_no_add = game_manager.get_instance_manager_no_add()->get_map_instance().get_province_instances();

		std::deque<ModifierEffect> const& modifier_effects = game_manager.get_definition_manager().get_modifier_manager().get_modifier_effects();
		...
		if (provinces.size() != provinces_no_add.size()) {
			...
			for (size_t idx = 0; idx < provinces.size(); ++idx) {
				...
				// ModifierSum const& modifier_sum = province.get_modifier_sum();
				// ModifierSum const& modifier_sum_no_add = province_no_add.get_modifier_sum();
				...
				for (ModifierEffect const& effect : modifier_effects) {
					...
					if (value != value_no_add) {
						Logger::error(
							...
							" at ID ", province.get_identifier(), " between add (", value.to_string(), ") and no-add (", value_no_add.to_string(), ") instances!"
						);
						ret = false;
						continue;
					}
					...
					std::vector<ModifierSum::modifier_entry_t> contributions_no_add = province_no_add.get_contributing_modifiers(effect);

					if (contributions.size() != contributions_no_add.size()) {
						Logger::error(
							"ProvinceInstance modifier effect contributing modifier count mismatch for effect ", effect.get_identifier(),
							" at ID ", province.get_identifier(), " between add (", contributions.size(), ") and no-add (", contributions_no_add.size(), ") instances!"
						);
						ret = false;
						continue;
					}
					for (size_t cidx = 0; cidx < contributions.size(); ++cidx) {
						...
						if (contribution != contribution_no_add) {
							Logger::error(
								"ProvinceInstance modifier effect contributing modifier mismatch for effect ", effect.get_identifier(),
								" at ID ", province.get_identifier(), " between add (", contribution.to_string(), ") and no-add (", contribution_no_add.to_string(), ") instances!"
							);
							ret = false;
							continue;
						}
